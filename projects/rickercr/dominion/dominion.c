...
int smithy_turn(int currentPlayer, struct gameState * state, int handPos) {
   //+3 Cards
   for (int i = 0; i < 3; i++)
   {
      drawCard(currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case remodel:
          return remodel_turn(currentPlayer, state, handPos,
                              choice1, choice2);
		
    case smithy:
       return smithy_turn(currentPlayer, state, handPos);
		
    case village:
          return village_turn(currentPlayer, state, handPos);
